#!/bin/sh
# Revisa que la versión sea la misma en src/config.h, res/jarvis.rc y
# res/jarvis.manifest y, si
...
for x in "$vw" "$fv" "$pv" "$fs" "$ps" "$mv"; do
    [ "$x" = "$v" ] || { echo "FALLA: la versión no es la misma en config.h, jarvis.rc y jarvis.manifest"; exit 1; }
done
if [ -n "$1" ] && [ "${1#v}" != "$v" ]; then
