...
void paths_init(void)
{
    ...
    g_paths.local_dir = path_join(local, L"Jarvis");
    free(local);

    /* Misma regla que la versión en Python, para seguir leyendo la memoria
       que ya tenías guardada: %OneDrive%\Desktop\Jarvis, o ~\Desktop\Jarvis. */
    ...
    g_paths.memory_dir = path_join(desk, L"Jarvis");
    free(desk);
    free(root);

    g_paths.config_file = path_join(g_paths.local_dir, L"config.env");
    g_paths.log_file = path_join(g_paths.local_dir, L"jarvis.log");
...
static bool save_locked(void)
{
    StrBuf sb;
    sb_init(&sb);
    sb_append(&sb, "# Configuración de Jarvis. Se edita desde la ventana de Configuración.\n");
...
/* La versión en Python guardaba .env, commands.json, dispositivos.json y
   ...
   versión, los copia a %LOCALAPPDATA%\Jarvis para que nada quede regado —
