...
/* Sokari: InitOnceExecuteOnce en lugar de una CRITICAL_SECTION armada a mano. */
static BOOL CALLBACK run_once(PINIT_ONCE once_ctl, PVOID param, PVOID *ctx) {
  ((void (*)(void))param)();
  return TRUE;
}

static void once(void (*func)(void)) {
  static INIT_ONCE lock = INIT_ONCE_STATIC_INIT;
  InitOnceExecuteOnce(&lock, run_once, (PVOID)func, NULL);
}
