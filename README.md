...
```bash
# Ubuntu (Fedora: sudo dnf install gcc make pkgconf-pkg-config libcurl-devel pulseaudio-libs-devel glib2-devel espeak-ng)
sudo apt install build-essential pkg-config libcurl4-openssl-dev libpulse-dev libglib2.0-dev espeak-ng
...
- **La malla (tus otras PCs):** igual que en Windows: escucha solo en la IP de Tailscale de esta PC (`tailscale0`), pide el mismo secreto o reconoce tu misma cuenta de Tailscale, y «Detectar» también encuentra PCs con Linux. Mientras llega la ventana: `sokari --revisar-malla`, `sokari --detectar-pcs` y `sokari --permitir-firewall`. Este último abre el puerto 8765 solo para tu red de Tailscale, en ufw (Ubuntu) o firewalld (Fedora), y pide tu contraseña. Tailscale normalmente deja pasar lo suyo aunque el firewall esté prendido, y Fedora Workstation ya trae ese puerto abierto.
